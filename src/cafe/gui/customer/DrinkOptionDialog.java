...
public class DrinkOptionDialog extends JDialog {
    ...
    public DrinkOptionDialog(Frame owner, MenuItem baseDrink) {
        ...
        lblPrice.setFont(new Font(FONT_NAME, Font.PLAIN, 12));
    ...
    private void processOrder(MenuItem item) {
        ...
        String suhu = inputPanel.getRbIce().isSelected() ? "Dingin" : "Panas";
        temp = new TemperatureDecorator(temp, suhu);

        int idx = inputPanel.getCbSize().getSelectedIndex();
        
        temp = switch (idx) {
            case 1 -> new SizeDecorator(temp, "Sedang", 3000);
            case 2 -> new SizeDecorator(temp, "Besar", 5000);
            default -> new SizeDecorator(temp, "Kecil", 0);
        };

        // FIX: Menggunakan getter untuk mengakses JCheckBox
        if (inputPanel.getChkBoba().isSelected()) temp = new ToppingDecorator(temp, "Boba", 3000);
        if (inputPanel.getChkCream().isSelected()) temp = new ToppingDecorator(temp, "Krim", 4000);
        if (inputPanel.getChkCheese().isSelected()) temp = new ToppingDecorator(temp, "Keju", 5000);
        if (inputPanel.getChkSugar().isSelected()) temp = new ToppingDecorator(temp, "Gula", 2000);
        
        this.resultItem = temp;
        this.quantity = (int) inputPanel.getSpinQty().getValue();
        this.confirmed = true;
        dispose();
    }
